...
local function isVersionCompatible(version1, version2)
    local function parseVersion(version_str)
        local major, minor, patch = version_str:match("^(%d+)%.(%d+)%.(%d+)")
				if not major then
            return nil, "Invalid version format: " .. version_str
        end
    ...
    -- Check if version1 >= version2 for all components
...
for _, data in pairs(remoteFiles) do
